...
window.SKACHKI_BREEDING = (function () {
  ...
  function canUseAsParent(horse, gender) {
    ...
    if (isExternalStud(horse)) return gender === 'stallion' && !ownAvailableParents('stallion').length;
    if (isExternalMare(horse)) return gender === 'mare' && !ownAvailableParents('mare').length;
  ...
  function renderPairIntro(stallion, mare) {
    ...
    if (usesStudService && usesMareService) {
      ...
      desc = 'Своих доступных жеребцов нет. Можно нанять племенного жеребца за ' + STUD_SERVICE_FEE + ' 🪙.';
    } else if (usesMareService) {
      desc = 'Своих доступных кобыл нет. Можно нанять племенную кобылу за ' + MARE_SERVICE_FEE + ' 🪙.';
    }
  ...
  function renderParentPicker(gender) {
    ...
    if (usesService && gender === 'stallion') {
      desc = 'Своих доступных жеребцов нет. Доступна племенная станция за ' + STUD_SERVICE_FEE + ' 🪙.';
    } else if (usesService && gender === 'mare') {
      desc = 'Своих доступных кобыл нет. Доступна племенная кобыла за ' + MARE_SERVICE_FEE + ' 🪙.';
    } else {
